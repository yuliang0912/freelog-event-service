...
const HshedWheelSlot = require('./hashed-wheel-slot')
...
module.exports = class hashedWheelTimer {
     ...
     */
    bulidWheel() {
        let queueLength = this.timeoutQueue.length

        this.timeoutQueue.forEach(item => {
            let wheekIndex = Math.floor(item.deadline / this.totalTimePerWheel)
            let slotIndex = Math.floor((item.deadline % this.totalTimePerWheel) / this.tickDuration)

            if (!this.allWheels.has(wheekIndex)) {
                this.allWheels.set(wheekIndex, new Map())
            }

            let targetWheel = this.allWheels.get(wheekIndex)
            if (!targetWheel.has(slotIndex)) {
                targetWheel.set(slotIndex, new HshedWheelSlot(wheekIndex, slotIndex))
            }
